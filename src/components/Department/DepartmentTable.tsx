...
interface Department { code: string; name: string; description: string; createdBy: string; createdOn: string; active: boolean; }
interface DepartmentTableProps { departments: Department[]; selectedDepartments: number[]; toggleCheckbox: (index: number) => void; toggleActive: (index: number) => void; setSelectedDepartments: (selected: number[]) => void; }
    ...
    <div className="bg-white rounded-lg p-4 mt-4">
      <table className="w-full border-collapse shadow-md rounded-lg">
        <thead><tr className="bg-gray-100">
          <th className="p-3 text-left"><input type="checkbox" onChange={(e) => e.target.checked ? setSelectedDepartments(departments.map((_, i) => i)) : setSelectedDepartments([])} checked={selectedDepartments.length === departments.length} /></th>
          <th className="p-3 text-left">Code</th><th className="p-3 text-left">Name</th><th className="p-3 text-left">Description</th>
          <th className="p-3 text-left">Created By</th><th className="p-3 text-left">Created On</th><th className="p-3 text-left">Active</th>
          <th className="p-3 text-left">Action</th>
        </tr></thead>
        <tbody>
          {departments.map((dept, index) => (
            <tr key={index} className="border-t hover:bg-gray-50">
              <td className="p-3"><input type="checkbox" checked={selectedDepartments.includes(index)} onChange={() => toggleCheckbox(index)} /></td>
              <td className="p-3">{dept.code}</td><td className="p-3">{dept.name}</td><td className="p-3">{dept.description}</td>
              <td className="p-3"><span className={`px-2 py-1 rounded-lg text-sm font-semibold ${dept.createdBy==='Owner'?'text-blue-500 bg-blue-50':dept.createdBy==='Member'?'text-dark-blue bg-blue-100':'text-black bg-gray-200'}`}>{dept.createdBy}</span></td>
              <td className="p-3">{dept.createdOn}</td>
              <td className="p-3"><Switch checked={dept.active} onChange={() => toggleActive(index)} /></td>
